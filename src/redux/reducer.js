...
const todoReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case REMOVE_TODO: // this for remove task from list
      swal({
        text: "Well Done 😀",
        icon: "success",
        buttons: false,
    ...
    case MARK_COMPLETED: // used to marked all task as completed
      localStorage.setItem('todos',JSON.stringify(state.todos.map((todo, index) =>
    ...
    case MARK_INCOMPLETE: // used to marked all task as completed
      localStorage.setItem('todos',JSON.stringify(state.todos.map((todo, index) =>
